...
def pre_process(id_: str, paths: dict, **kwargs) -> structure.BrainImage:
    ...
    if kwargs.get('resampling_pre', False):
        pipeline_t1.add_filter(fltr_prep.Resampling(new_spacing=(0.8,0.8,0.8),method='Bspline'))
    ...
    if kwargs.get('resampling_pre', False):
        pipeline_t2.add_filter(fltr_prep.Resampling(new_spacing=(0.8,0.8,0.8),method='Bspline'))
    ...
    if kwargs.get('resampling_pre', False):
        pipeline_resampling.add_filter(fltr_prep.Resampling(new_spacing=(0.8,0.8,0.8),method='NN'))
    ...
    if kwargs.get('resampling_pre', False):
        pipeline_gt.add_filter(fltr_prep.Resampling(new_spacing=(0.8,0.8,0.8),method='NN'))
